...
erDiagram
    ...
    device_config ||--o{ locations : "devid"
    device_config ||--o{ device_access : "devid"

    auth_users ||--o| profiles : "id"
    auth_users ||--o| user_roles : "user_id"
    auth_users ||--o{ device_access : "user_id"
    ...
    device_config {
        ...
        timestamp device_data_updated_at
        timestamp last_seen
        timestamp created_at
        int last_uplink_count
        int battery_level
...
**Key Relationships:**
- **One device** → **many** activity/reboot/sensor_data/locations records
- **One user** → **one** profile, **one** role, **many** device access permissions
- All tables except `device_config` use UUID primary keys
...
CREATE TABLE device_config (
  ...
  application_mode INTEGER CHECK (application_mode >= 0 AND application_mode <= 3),
  device_data_updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  last_seen TIMESTAMP WITH TIME ZONE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  last_uplink_count INTEGER,                   -- Last received uplink counter
  battery_level INTEGER,                       -- Battery level as percentage (0-100)
  internal_temperature INTEGER,                -- Internal modem temperature in Celsius
  sensor_type INTEGER DEFAULT 0                -- 0 = tracker, 1 = soil sensor
);
...
- Updated automatically when heartbeat messages are received via triggers
...
Dedicated table for device location data with coordinates and accuracy metrics.
...
CREATE TABLE locations (
  ...
  location_type TEXT NOT NULL,         -- Type: 'wifi', 'gnss', 'cell', 'here_api'
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL,
  uplink_count INTEGER,                -- Message counter
  test_column JSONB                    -- Temporary field for testing
);

-- Indexes for spatial and time-based queries
CREATE INDEX idx_locations_devid_created ON locations(devid, created_at DESC);
CREATE INDEX idx_locations_created_at ON locations(created_at DESC);
CREATE INDEX idx_locations_coords ON locations(latitude, longitude);
```

**Location Types:**
- `wifi`: WiFi-based positioning via HERE API
- `gnss`: GPS/GNSS positioning
- `cell`: Cell tower-based positioning
- `here_api`: Combined positioning using HERE Positioning API
...
CREATE TABLE activity (
  ...
  devid TEXT NOT NULL REFERENCES device_config(devid),
  uplink_count INTEGER,                -- Message counter
  sleep INTEGER,                       -- Sleep time in seconds
  modem INTEGER,                       -- Modem active time in seconds
  gnss INTEGER,                        -- GNSS active time in seconds
  wifi INTEGER,                        -- WiFi scanning time in seconds
  other INTEGER,                       -- Other activities in seconds
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
- Stores additional user metadata beyond authentication

### 7. user_roles

User access control and permissions using custom enum type.

```sql
-- Custom enum for role types
CREATE TYPE app_role AS ENUM ('admin', 'moderator', 'user', 'developer');

CREATE TABLE user_roles (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id),
  role app_role NOT NULL,              -- Enum: 'admin', 'moderator', 'user', 'developer'
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL,
...
User-to-device access permissions for granular access control.
...
CREATE INDEX idx_device_access_device ON device_access(devid);
CREATE UNIQUE INDEX idx_device_access_unique ON device_access(user_id, devid);
```

**Purpose:**
- Allows fine-grained control over which users can access specific devices
- Used in combination with Row Level Security policies
- Admins and moderators typically have access to all devices
